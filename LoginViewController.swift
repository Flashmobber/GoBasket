...
class LoginViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func unwindToLoginFromIntro(_ segue: UIStoryboardSegue) {
        // возврат с Intro
    }
    ...
    fileprivate func setLoginBtnAttr(isEnabled: Bool, backgroundColor: String, titleColor: String, state: UIControl.State) {
        loginBtn.isEnabled = isEnabled
        loginBtn.backgroundColor = UIColor(named: backgroundColor)
        loginBtn.setTitleColor(UIColor(named: titleColor), for: state)
    }
    
    @IBAction func editingChanged(_ sender: UITextField) {
        if login.text!.count >= 17 && !password.text!.isEmpty {
            setLoginBtnAttr(isEnabled: true, backgroundColor: "#buttonEnable", titleColor: "#buttonEnableText", state: .normal)
        } else {
            setLoginBtnAttr(isEnabled: false, backgroundColor: "#buttonDisable", titleColor: "#buttonDisbleText", state: .disabled)
        }
